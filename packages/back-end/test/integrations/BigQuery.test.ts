...
describe("getExperimentAggregateUnitsQuery dimension slicing", () => {
  let integration: BigQuery;
  ...
  const exposureQuery: ExposureQuery = {
    ...
    description: "",
    query:
      "SELECT user_id, timestamp, experiment_id, variation_id, country, release_channel FROM exposures",
    userIdType: "user_id",
    dimensions: ["country", "release_channel"],
  };
  ...
  it("does not emit IN () when a dimension has empty specifiedSlices", () => {
    const sql = integration.getExperimentAggregateUnitsQuery({
      settings,
      activationMetric: null,
      segment: null,
      factTableMap: new Map(),
      dimensions: [{ type: "experiment", id: "country", specifiedSlices: [] }],
      useUnitsTable: false,
    });
  ...
  it("emits IN (...) with slice values when specifiedSlices is non-empty", () => {
    const sql = integration.getExperimentAggregateUnitsQuery({
      settings,
      activationMetric: null,
      segment: null,
    ...
    expect(sql).not.toMatch(/IN\s*\(\s*\)/);
    expect(sql).toMatch(/IN \('US',\s*'CA'\)/);
    expect(sql).toContain("AS dim_exp_country");
  });

  it("handles mixed dimensions where only some have specifiedSlices", () => {
    const sql = integration.getExperimentAggregateUnitsQuery({
      settings,
      activationMetric: null,
      segment: null,
    ...
    expect(sql).not.toMatch(/IN\s*\(\s*\)/);
    expect(sql).toContain("IN ('US')");
    // Only the sliced dimension should get a CASE-wrapped "AS dim_exp_*"
    // projection; the un-sliced one passes through as a bare column reference.
    expect(sql).toMatch(/END\s*\)\s*AS dim_exp_country/);
    expect(sql).not.toMatch(/END\s*\)\s*AS dim_exp_release_channel/);
    expect(sql).toMatch(/,\s*dim_exp_release_channel\s*\n/);
  });
});
